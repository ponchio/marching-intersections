...
// using Vec3, Pos3, Box3f from lib/vec.h and lib/box.h

void loadObj(QString filename, std::vector<Vec3> &vert, std::vector<int> &face) {
    char buffer[1024];
    QFile file(filename);
...
void saveObj(QString filename, std::vector<Vec3> &vert, std::vector<Vec3> &norm, std::vector<int> &face) {
    QFile file(filename);
...
void savePly(QString filename, std::vector<Vec3> &vert, std::vector<Vec3> &norm, std::vector<int> &face) {
...
void savePlyBinCloud(QString filename, std::vector<Vec3> &vert, std::vector<Vec3> &norm) {
...
void savePlyBinMesh(QString filename, std::vector<Vec3> &vert, std::vector<int> &face) {
...
void savePlyAsciiMesh(QString filename, std::vector<Vec3> &vert, std::vector<int> &face) {
...
void savePlyAsciiQuadMesh(QString filename, std::vector<Vec3> &vert, std::vector<int> &face) {
...
void rescale(std::vector<Vec3> &vert, float factor) {
    for(size_t i = 0; i < vert.size(); i++)
        vert[i] *= factor;        
}

void translate(std::vector<Vec3> &vert, Vec3 t) {
    for(size_t i = 0; i < vert.size(); i++) {
        vert[i] += t;        
    }
}

void rotate(std::vector<Vec3> &vert) {
    for(size_t i = 0; i < vert.size(); i++) {
        //vert[i] = Pos3( vert[i].Y(),-vert[i].X(),vert[i].Z() );
        vert[i] = Pos3( vert[i].X(),-vert[i].Z(),vert[i].Y() );
    }
}

float diagLen(const std::vector<Vec3> &vert){
    Box3f box;
...
Vec3 center(std::vector<Vec3> &vert) {
    Box3f box;
    for(uint i = 0; i < vert.size(); i++)
        box.Add(vert[i]);
...
void addNoise(std::vector<Vec3> &vert){
    //int t = time(NULL);
    //srand(time(NULL));
    srand(1409248851);
...
public:
    ...
    Box3f box() {
        Pos3 diag(radius, radius, radius);
        Box3f box;
        box.min = -diag;
        box.max = diag;
        return box;
    ...
    std::vector<float> getIntersections(int /*plane*/, float u, float v,std::vector<Vec3> &norms) {
        std::vector<float> result;
        float d = radius*radius - u*u - v*v;
        if(d > 0) {
            d = sqrt(d);
            result.push_back(-d);
            norms.push_back( Vec3(u,v,-d).normalized() );
            result.push_back(d);
            norms.push_back( Vec3(u,v,+d).normalized() );
        }
        return result;
...
void testImplicitSphere(){
    ...
    vol.intersectPlane( Vec3(1,2,1), -Vec3(0.3,0.3,0.25));
    vol.intersectPlane( Vec3(1,-2.5,1), -Vec3(0.33,-0.32,0.25));

    ImplicitSphere sphere2(radius*1.2);
    mi::Volume vol2;
    vol2.fromImplicit(sphere2, step);
    vol2.translate( Vec3i(12,12,2) );

    mi::Volume vol3;
    vol3.unification(vol,vol2);

    vol3.toMeshDual(vert, face);
    savePlyAsciiQuadMesh("per_callieri.ply", vert, face);
}
...
void testSweep(){
    ...
    mi::Volume vol2;
    vol2.sweep(vol1, Vec3(0, 0, 0), Vec3(1.3, 1.3, 1.3), 1);
...
void testSweepMesh(){
    ...
    mi::Volume vol2;
    Vec3 dest = Vec3(0.312, 0.23,-0.33)*diagLen(vert);
    //vol2.fastSweep( vol1, Vec3(0, 0, 0), dest*4 ,4);
    vol2.sweep( vol1, Vec3(0, 0, 0),  dest ,4 );
...
void testCSM(){
    ...
    //volume2.fromSweep(vert, face, Vec3(0, 0, 0), Vec3(0.3, 0.3, 0.3), step);
    ...
    //volume3.intersectPlane( Vec3(1,2,1).normalized(), Vec3(0,0,0) );
